...
proc playgame {} {
	...
	for {set n 0} {$n<$numciv} {incr n} {
		set col [rndcol]
		while {[coldif $col $green]<50 || [coldif $col $blue]<50} {set col [rndcol]}
		...
		while {$on} {
			foreach land $activeland {
				...
				if {$col!=$green && $col!=$blue} {
					...
					if {rand()<$rebchance} {set col [rndcol]}
					if {rand()<$changchance} {set ncol [mutcol $col]} else {set ncol $col}
					#target occupied land
					if {$coldif>30 && $targcol!=$blue && $targcol!=$green && rand()<$winchance} {
						lappend activeland $targ
						set army($targ) $ncol
					...
					#target empty land
						...
						.map  itemconfigure cell($targ) -fill $ncol -outline $ncol
					#target occupied sea
					} elseif {$targcol==$blue && $navy($targ)!=$blue && rand()<$winchance} {
...
.map  itemconfigure cell($sea) -fill $blue -outline $blue
			...
			#}
			if {$activeland==""} {set activeland $territory;set activesea $navies;incr count}
...
label .me -text "Map maker"
grid .me -in .bottom -row 1 -column 5

label .me2 -text "by Alex Braithwaite"
grid .me2 -in .bottom -row 2 -column 5
